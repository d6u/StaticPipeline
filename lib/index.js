...
export default class StaticPipeline {
  ...
  async _runTask(taskName) {
    ...
    if (!task.files) {
      await this._dispatchTask({}, task.process, { log, taskName, glob: false });
      return;
    }

    for (let file of task.files) {
      let fileObjs = await resolveFile(file, this.opts);
      let glob = typeof file.base !== 'undefined';

      for (let fileObj of fileObjs) {
        await this._dispatchTask(fileObj, task.process, { log, taskName, glob });
      }
    }
  }
  ...
  async _dispatchTask(fileObj, taskProcess, opts) {
    ...
    let runBlock = async function () {
      return await self._processTask(fileObj, taskProcess, opts);
    };

    let watchList = await runBlock();
   ...
   * @param {Object} [opts] - Additional options
   * @param {bool} [opts.glob] - Indicate if this fileKey is generated based on a glob pattern. Must be specified if creating new record in `_watchMap`
   * @return {void}
   */
  _updateWatchTask(taskName, fileKey, files, runBlock, opts) {
    ...
    for (let file of oldFiles) {
      ...
      for (let [_taskName, fileMap] of this._watcherMap) {
        for (let [_fileKey, obj] of fileMap) {
          if (_taskName === taskName && _fileKey === fileKey) continue;
    ...
    for (let [taskName, fileMap] of this._watcherMap) {
      for (let [fileKey, obj] of fileMap) {
        ...
        if (fileKey === file) {
          ...
          this._updateWatchTask(taskName, fileKey, [], obj.runBlock);
          continue;
        }

        if (obj.files.indexOf(file) > -1) {
          let filesLeft = difference(obj.files, [file]);
          this._updateWatchTask(taskName, fileKey, filesLeft, obj.runBlock);
        }
