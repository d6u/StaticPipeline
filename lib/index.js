...
/**
 * A base class for StaticPipeline class to extends. In StaticPipeline
 * constructor, each method will be wrapped with Bluebird.coroutine so
 * generators can play nicely with Promises
 */
export class StaticPipelineBase {
...
export default class StaticPipeline extends StaticPipelineBase {
  ...
  constructor(tasks, opts) {
    ...
    this._run = Bluebird.coroutine(this._run);
    this._runTask = Bluebird.coroutine(this._runTask);
    this._dispatchTask = Bluebird.coroutine(this._dispatchTask);
    this._processTask = Bluebird.coroutine(this._processTask);
  }
  ...
  _updateWatchTask(taskName, fileKey, files, runBlock) {
    ...
    if (!this._watcherMap[taskName]) {
      this._watcherMap[taskName] = {};
    }

    if (!this._watcherMap[taskName][fileKey]) {
      this._watcherMap[taskName][fileKey] = {
        files: files,
        runBlock: runBlock
      };
    ...
    let curFiles = this._watcherMap[taskName][fileKey].files;
    ...
    this._watcherMap[taskName][fileKey].files = files;
  }

  _fileChanged(file) {
    let found = false;

    Object.keys(this._watcherMap).forEach( taskName => {
      Object.keys(this._watcherMap[taskName]).forEach( fileKey => {

        if (this._watcherMap[taskName][fileKey].files.indexOf(file) > -1) {
          found = true;

          let runBlock = this._watcherMap[taskName][fileKey].runBlock;
          runBlock().then(watchList => {
