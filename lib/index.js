...
export default class StaticPipeline {
  ...
  constructor(tasks, opts) {
    ...
    if (this.opts.watching) {
      ...
      this._watcher.on('add', path => this._fileAdded(path));
    }
  }
  ...
  async _processTask(fileObj, processBlock, opts) {
    ...
    return await Bluebird.fromNode(function (resolver) {
   ...
   * @return {void}
   */
  _updateGlobWatchTask(taskName, fileObj, processBlock) {
    let globPattern = path.resolve(this.opts.workingDir, fileObj.base, fileObj.src);
  ...
  _fileAdded(file) {
    for (let [taskName, detail] of this._watchGlobMap) {
      if (minimatch(file, detail.globPattern)) {

        let dest = file.replace(
          path.resolve(this.opts.workingDir, detail.fileDef.base),
          path.resolve(this.opts.workingDir, detail.fileDef.dest));

        let fileObj = { src: file, dest };

        this._dispatchTask(fileObj, detail.process, {
          taskName,
          log: this.opts.logging ? createLogger(taskName) : noop,
          isGlob: true
        });
