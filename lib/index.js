...
import { AssetNotFoundError, ConfigNotFoundError, FileNotFoundError } from './errors';
...
export default class StaticPipeline {
   ...
   * @param {bool} [opts.assets.useMap=true] - If false, assets helper will do
   *                                         nothing but return whatever passed
   *                                         in.
   * @param {bool} [opts.assets.forceMap=false] - If true, will throw an error
  ...
  start() {
    return Bluebird.coroutine(this._run.bind(this))();
  }

  * _run() {
    let queue = resolveTaskDependencies(this.tasks);
  ...
  * _processTask(fileObj, processBlock, opts) {
  ...
  setAsset(dest, hashedDest) {
    if (!this.opts.assets.useMap) return;
  ...
  assets(url) {
    if (!this.opts.assets.useMap) return url;

    if (!url) return this._assetMap;

    let hashedUrl = this._assetMap[url];
