...
export default class StaticPipeline {
  ...
  * _processTask(fileObj, processBlock, opts) {
  ...
  _fileChanged(file) {
    ...
    for (let taskName of Object.keys(this._watcherMap)) {
      for (let fileKey of Object.keys(this._watcherMap[taskName])) {
        if (this._watcherMap[taskName][fileKey].files.indexOf(file) > -1) {
          runBlock = this._watcherMap[taskName][fileKey].runBlock;
          _taskName = taskName;
          _fileKey = fileKey;
          break;
        }
    ...
    if (runBlock) {
      runBlock().then(watchList => this._updateWatchTask(_taskName, _fileKey, watchList, runBlock));
    } else {
