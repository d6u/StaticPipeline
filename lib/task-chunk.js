    ...
    return p.bind(this)
      .then(function (h) {
        var finalDest = pair.dest.replace(EXT_REGEX, '-' + h.hash + '$&'),
          unhashed = pair.dest.replace(path.resolve(this.config._publicBase), ''),
          hashed = finalDest.replace(path.resolve(this.config._publicBase), this.config._newBase || '');
        this.assetsDict[unhashed] = hashed;
        return finalDest;
      });
  }.bind(this);
};
